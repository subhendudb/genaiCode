...
class Transaction(db.Model):
    ...
    id = db.Column(db.UUID(as_uuid=True), primary_key=True, server_default=db.text('gen_random_uuid()'))
    account_id = db.Column(db.UUID(as_uuid=True), db.ForeignKey('accounting.accounts.id'), nullable=False)
    contra_account_id = db.Column(db.UUID(as_uuid=True), db.ForeignKey('accounting.accounts.id'), nullable=False)
    ...
    contra_account = db.relationship('Account', foreign_keys=[contra_account_id], backref='contra_transactions')
    ...
    def to_dict(self):
        ...
        return {
            ...
            'contra_account_name': self.contra_account.name if self.contra_account else None,
...
class AccountCreate(BaseModel):
    ...
    type: str = Field(..., pattern='^(INCOME|EXPENSE|ASSET|LIABILITY)$')  # <-- changed here
...
class AccountService:
    ...
    def create_account(data: dict) -> Account:
        ...
        try:
            ...
            db.session.rollback()
            raise RequestValidationError("Account creation failed due to database constraints")
    ...
    @staticmethod
    def list_accounts(filters: dict = None, page: int = 1, per_page: int = 20) -> List[Account]:
        """
...
class TransactionService:
    ...
    @staticmethod
    def list_transactions(filters: dict = None, page: int = 1, per_page: int = 20) -> List[Transaction]:
        """
...
class ReportService:
    ...
    def generate_profit_loss_report(start_date: date, end_date: date) -> dict:
        ...
        try:
            ...
            expenses = expenses_result if expenses_result is not None else Decimal('0.00')
...
def login():
    ...
    try:
        ...
        payload = {
            'sub': user.username,
            'iat': datetime.utcnow(),
            'exp': datetime.utcnow() + timedelta(seconds=app.config['TOKEN_EXPIRES_SECONDS'])
        }
        token = jwt.encode(
            payload,
        ...
        return jsonify({
            'token': token,
            'expires_in': app.config['TOKEN_EXPIRES_SECONDS']
        })
    except ValidationError as e:
