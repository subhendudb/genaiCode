...
test.describe('Transaction Management', () => {
  ...
  test('US-004: Record transactions', async ({ page }) => {
    ...
    await page.selectOption('select[name="account_id"]', { label: 'Bank Account (ASSET)' });
    await page.selectOption('select[name="contra_account_id"]', { label: 'Office Expenses (EXPENSE)' });
    ...
    await expect(page.locator('text=Bank Account').locator('..').locator('.account-balance')).toHaveText('$850.00');
  });
  ...
  test('US-005: Prevent negative balances', async ({ page }) => {
    ...
    await expect(page.locator('.Toastify__toast--error')).toContainText('Insufficient funds');
    
    // Verify balance unchanged
    await page.goto('http://localhost:8000/accounts');
    await expect(page.locator('text=Bank Account').locator('..').locator('.account-balance')).toHaveText('$1,000.00');
  });

  test('US-004: Edge cases - duplicate and future transactions', async ({ page }) => {
    ...
    await page.fill('input[name="description"]', 'Duplicate test');
    await page.click('button:has-text("Record Transaction")');
    await expect(page.locator('.Toastify__toast--warning')).toContainText('possible duplicate');
